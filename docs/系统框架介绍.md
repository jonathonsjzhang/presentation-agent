...
1. 框架优化：
- **Skill 原子化拆解**：将各 Agent 的 skill 按汇报对象（董事会、总办、战略负责人、业务团队、外部）、汇报性质（专题深度分析与信息快速同步）和材料格式（文档、PPT、HTML）拆成多个原子能力。Agent 调用时根据 brief 参数自由组装组合，减少单次任务的上下文漂移。
- **多Agent 和 Sub_agent机制**：当前的生成和 review 实际由同一个 agent 进行，在不同阶段使用不同skill，但存在上下文积累，可能导致上下文漂移问题。
- **执行问题**：当前Agent运行速度较慢、容易中途卡顿、截断，需要debug确认原因。
- **项目模式迁移**：尝试如何将插件系统迁移到WB项目模式上。

2. 知识迭代：
- **Skill 多轮迭代**：当前 7 个 Agent 的 skill 包均为初版，需在实际使用中持续迭代 SOP 和 rubrics，提升输出质量与稳定性。
