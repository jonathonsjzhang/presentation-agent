...
> 汇报助手是一套面向战略汇报生产的 7-Agent Loop Harness：它把汇报材料从任务定位、论点提炼、故事线、单页内容、格式化、Q&A 到逐字稿拆成 7 个独立 Agent；每个 Agent 由可编辑 skill 定义工作方式，由 loop 执行、review 拦截、state/memory 持续学习，并通过 Web Cockpit 可视化管理整个 harness。
...
是的，当前方案需要将项目代码托管到一个 Git 远程仓库。建议使用 GitHub 私有或公开仓库。用户首次安装时，Agent 会通过以下优先级获取仓库地址：

1. 环境变量 `PRESENTATION_AGENT_REPO_URL`（供企业内部批量部署）
2. 用户在对话中直接提供（如"安装汇报助手，地址是 https://github.com/xxx/xxx"）
3. Agent 主动向用户索要

仓库地址配置为一次性操作。安装完成后地址被记录在 host adapter skill 或环境变量中，后续使用与更新不再需要重复提供。
