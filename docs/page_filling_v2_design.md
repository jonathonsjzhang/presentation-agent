...
"evidence_steps": [
  {
    ...
    "quant": {                                 // 新增（可选）
      ...
      "delta": "+14pp",                        // 差值/相对变化
      "metric": "30日强留存率",                // 口径
      "unit_scope": "元宝 / 2025Q2 / n=1,243"  // 实体·时间·样本
    }
  }
]
```
...
v2 不是孤立改动，下游 format 必须同步规划（但可滞后执行）：

| 影响项 | 现状 | v2 后动作 | 时机 |
|---|---|---|---|
| `skills/format/schemas/page_content.v1.json`（镜像） | 宽松浅校验 | 暂不收紧 → 不会报错 | 并行期不动 |
| format SKILL.md 输入消费 | 读 v1 字段 | 增加读 `comparison_matrix` / `qualitative_evidence` / `quant` 的指引 | 阶段二 |
| format rubrics | 按 v1 | 增加"对比矩阵是否渲染成表/图""用户原声是否进 callout" | 阶段二 |
| format invariant | 不改上游结论 | 不变（v2 兼容） | — |

**迁移节奏**：
1. 阶段一（本次）：page_filling 改 v2，format 不动（靠浅校验跑通），人工验证 v2 产出质量。
2. 阶段二（后续）：format 升级消费 v2 新字段，让对比矩阵/用户原声真正落到版面。
...
```
1. 新建 schemas/page_content.v2.json（v1 超集 + §3 四增量）
2. 改 SKILL.md（§5：workflow 0、触发器、3 条 invariant、output 改 v2）
3. 升级 rubrics.json（§6：保留 4 条 + 新增 6 条，density 走 warning）
4. 建 references/（argument_chain / information_sufficiency / page_archetypes / gotchas）
5. 建 examples/retention_manual_vs_ai.md
6. 跑一次留存案例验证 v2 产出（不改 format，观察是否变厚）
7. 若仍缺数据 → 触发 §1.4 输入侧回查
```
