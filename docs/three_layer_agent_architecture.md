...
> 状态：设计补充文档（针对 README 第八章 TODO #1「增加 Manager agent」与 TODO #2「多 Agent 和 Sub-agent 机制」）
> 关联文档：`docs/manager_agent_development_plan.md`（本地 harness 实现方案）、`README.md`、`configs/agents.json`
> 定位：本文定义**架构与 Agent 边界**；`manager_agent_development_plan.md` 定义**代码落地**。两者互补，本文为上游规范。
>
> **两条硬性设计目标（本次确认）：**
...
- [ ] `skills/report_builder/SKILL.md`：写入 §8.3 的 `SPAWN_WORKER`/`SPAWN_REVIEWER` 能力契约 + 三家映射速查表。
- [ ] 编写 `agents.json → 三套终端 agent 文件` 的生成脚本（`.workbuddy/agents` / `.claude/agents` / `.codex/agents`）。
- [ ] 在 SKILL.md 写明「派生只有一层、worker 写操作限工作区」两条迁就 Codex 的统一约束。
- [ ] 各终端各跑一次单环节 worker→review→P0 闭环（§3）做可移植性验证。
