...
- **Manager + 6 Worker Agent**：Manager 直接面向用户，负责任务定义、计划、派发、验收和返工；论点、故事线、单页、格式、Q&A、逐字稿由 6 个专业 Worker 在独立任务上下文中执行。
...
- **可组合原子能力**：6 个 Worker 按受众、汇报性质和材料格式编译当轮所需 Skill bundle，支持 fingerprint、prompt budget 与 legacy fallback。
- **投影上下文与 scoped memory**：Manager 按 Worker 投影上游 artifact；memory 按 capability owner 和 profile scope 检索，并可晋升到对应 facet rubric。
...
1. **框架优化**
   - **真实模型评测**：在静态预算、270 组合编译和 renderer 回归基础上，持续执行旧/新盲评与真实 run token 采样。
   - **项目接入**：当前主要通过自行部署 Git 仓库和 Host Skill 使用，安装、升级与版本管理仍有成本；后续需要评估接入 WorkBuddy 官方插件库或统一插件分发体系。

2. **知识迭代**
   - **持续迭代 SOP 与 rubrics**：当前 Manager 与 6 个 Worker 的 Skill 仍是草稿版，需要结合真实汇报任务持续校准职责边界、工作流、验收标准和返工策略。
...
Manager 根据任务选择 Worker，默认依赖仍可保持论点 → storyline → 单页 → format → Q&A / 逐字稿，但不再由固定 `PipelineStepper` 决定顺序。
...
```text
planning → human plan gate → dispatch worker → acceptance
...
- `presentation_agent/pipeline.py`：保留的六 Worker 固定顺序兼容/调试入口。
...
6 个 Worker 会确定性编译 `core worker + audience facet + report-type facet + format facet`，并把 fingerprint、规则集合和 prompt budget 固化到 run 目录。Manager 新任务默认使用 projected context：上游 artifact 按来源命名空间化，只内联当前 Worker 需要的字段；超大字段保留预览和文件引用。Format Worker 一次只加载 PPT、DOCX 或 HTML 其中一套执行规则。
...
`actor=human` 时，用户确认调用 `report approve`；用户要求调整或回答 Manager 问题调用 `report feedback`。Worker 内部仍由 `StepRunner` 完成 gen/review/revise，但调度权属于 Manager。
...
| 视图 | 作用 |
|---|---|
| Loop Map | 展示 Manager 控制面、6 个 Worker 和双层 loop |
| Inline Run | 演示单步执行流程 |
...
│   ├── skills/                     # Manager + 6 Worker skill + report_builder
...
| `report approve` | 确认 Manager 的 plan/final gate | 派发首任务或完成项目 |
