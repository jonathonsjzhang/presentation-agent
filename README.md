...
> 当前版本：`v0.4`（唯一可执行协议，Markdown-first 简化主链）
...
| Worker | Analysis（观点发现与收敛） | 通读参考材料，发散可能的发现、解释与假设，经过比较、追问和证据检验后，收敛成完整观点与候选主论点 | `analysis.md` + 轻量回执 |
...
v0.4 的默认生产依赖固定为 `analysis → storyline → report → qa_preparation → format`。Manager 可以派发返工，但不能把 Evidence 插入五阶段生产主链；Evidence 在有原始材料时作为 Brief 前的 run-level 输入处理任务运行，生成的 Catalog 由 Brief Gate 和 Analysis 共同复用。逐字稿 worker 已删除，不再出现在默认链路、扩展 gate 或 task packet 枚举中。
...
```text
brief gate → Analysis → analysis gate → Storyline → storyline gate
  → Report → QA → Format(real render) → final human gate
```

Worker 内部：

```text
读取完整上下文 → 按 Skill 工作 → 自检 → 文件/引用确定性校验 → 提交
```

Worker 负责按本专业 Skill 自检；runtime 负责文件、Schema、引用和真实渲染等可确定检查；Manager 负责对照项目目标和跨任务依赖验收。Analysis 与 Storyline 的方向由用户 gate 确认。
...
v0.4 把固定路由 bookkeeping 从 Manager 输出中收回 runtime：正常 happy path 由 runtime 自动推进；显式返工只需指定责任阶段和反馈，canonical Markdown 路径与 task packet 由 runtime 绑定。决策应用失败时，提交事务会回滚到可重试状态，不会停在半提交节点。

### 4.1 Runtime 模块

Runtime 是 loop 的运行骨架，主要由以下文件实现：

- `presentation_agent/loop.py`：`LoopRunner`，v0.4 单 Worker 调试入口；默认 Manager 链不使用独立 Reviewer。
...
每个 Agent 的 skill 包只保留行为定义和必要的结构契约：
...
这种设计的好处是：专业判断集中在唯一权威 `SKILL.md`，结构交接集中在 schema，确定性文件和渲染质量由 runtime 负责。历史版本留在 Git，不在 skill 目录堆 backup、draft 或整篇注入的 reference。
...
开头的 Brief gate 会确认七类信息：项目需求（研究背景、当前研究 hypo）、高可信论据、报告主题、听众、项目类型、交付形式和报告篇幅。首次 Brief gate 固定在一次 `AskUserQuestion` 中呈现 4 题：研究背景、当前研究 hypo、高可信论据、Brief 是否准确；前三题即使草案已有推断值也必须询问，且保持 `options=[]`，宿主不得添加“用户提供/用户填写”占位选项。runtime 同时返回 `presentation_required_before_tool=true`、独立的 `presentation_text`、`presentation_delivery_mode=separate_user_visible_message_before_tool` 和 `host_action_sequence=["send_present_to_user_message", "call_AskUserQuestion"]`，要求 WorkBuddy 先用一条不带 tool call 的独立用户可见消息完整展示 Brief，确认消息已展示后再调用结构化提问工具；不能把 Brief 仅放在同时含 tool call 的 response content 中，不能用简短过渡语替代，也不能把 Brief 塞进某一道题。四题答案通过 `report feedback` 以结构化 JSON 写回 `raw_brief.json`，其中 `brief_confirmed=true` 代表用户已在同一面板明确确认，无需再弹第二个确认面板。每次 human gate 都返回 `interaction_required`、`preferred_tool=AskUserQuestion` 和可直接透传的 `ask_user_question_payload`；WorkBuddy host 必须实际调用该工具，不能只展示问题文本。未获得明确确认时，runtime 拒绝 approve。研究背景与当前研究 hypo 不从 `decision_goal` / `expected_action` 等字段预设；高可信论据由 agent 根据输入材料整理成 evidence list，并让用户填写编号、名称或原文片段。报告主题由 Manager 根据输入与论据总结，听众/项目类型/交付形式/报告篇幅则从用户原始输入提取，缺省为总办、分析类、文档、3 页；若用户选择 PPT，篇幅缺省为 10 页 PPT。Brief 还展示固定 agent 执行流程 `analysis（分析） → storyline（故事线） → report（报告产出） → qa_preparation（追问清单） → format（可视化排版）`，不再展示或询问 reviewer/full_auto 选项。运行时默认在 Analysis 和 Storyline 完成后各暂停一次：Analysis gate 让用户确认主论点组，若用户选择“都不好”或“我自己修改”会复用当前 Analysis task 修订并再次确认；Storyline gate 以“序号、章节、标题（Leadline）、核心论证”四列表格展示 worker 产出的真实章节故事线，用户确认后进入 Report，若选择“不好，重新写”或“我自己修改”会复用当前 Storyline task 修订并再次确认。

报告篇幅采用“目标页数 + 一页自动容差”的分层控制。Brief 中确认的页数仍是写作目标，字符预算只用于生成引导和溢出预警；Report/Format 以正文影子稿的真实渲染页数为最终依据。实际正文不超过目标页数时正常通过，超出 1 页时由 runtime 自动放宽并继续，超过自动容差后不得继续机械返工或静默放宽，而是进入 `page_budget` human gate，让用户在“接受当前篇幅”和“继续收窄”之间明确选择。用户接受后，批准后的新上限写回项目 `state.json`；用户选择收窄时，复用当前 Report/Format task 返工，并保持原上限。

Core Skill 会与受众、报告类型和目标载体的精简 atomic capability 组合：例如面向董事会时更强调决策取舍，专题汇报要求完整候选论点比较；DOCX/PPT/HTML 的分页、布局与视觉质量则由各自 renderer/runtime 处理。
...
### 4.3 Runtime Validation 与用户 Gate

默认 v0.4 主链不派发独立 Reviewer sub-agent，也不加载生产 rubrics。质量责任分为三层：

1. **Worker 自检**：依据本环节唯一的 `SKILL.md` 检查专业判断、角色边界和成稿质量。
2. **Runtime Validation**：确定性检查提交文件、输出 schema、引用绑定、renderer 数据模型和真实交付文件；这些检查由代码和实际渲染结果负责。
3. **用户 Gate**：Analysis 论点组、Storyline 单版和最终材料由用户确认方向与可用性。

最终材料的独立 E2E Eval 仍然保留，并使用 `evals/rubrics/` 下版本化的评测标准。它是交付后 benchmark，不会反向塞进生产 Worker prompt。

### 4.4 E2E Eval Harness

生产链只做 Worker 自检与 runtime 确定性验证；E2E Eval Harness 独立评价最终 PPT、DOCX 或 HTML 是否达到正式汇报质量。评测 rubric 与生产 Skill 分离，避免生成规则因 benchmark 而膨胀或过拟合。
...
| 文件 | 职责 | 在链路中的位置 |
|---|---|---|
| `presentation_agent/memory.py` | 写入、提炼与整理（dreaming） | 入口 + 沉淀 + 维护 |
...
另外，与 agent memory 平行还存在一类**项目 state**（每个 run 下的 `state.json`）：它记录 Manager 定义、跨 Worker 共享的硬约束（受众、目标 action、Executive Summary、目标页数、自动容差和用户批准的新上限）。它和环节 memory 机制相同但作用域不同，**环节 memory 不覆盖项目约束**。
...
### 5.7 Memory 不自动固化为生产规则

同一条经验命中次数高，只说明它在若干历史任务中反复出现，不足以证明它应成为所有场景的硬规则。系统不再把 memory 自动写入 Skill 或检查清单：

```text
log（可追溯）→ scoped memory（按场景召回）→ 跨案例人工复盘 → 必要时精炼进 SKILL.md
```

这样避免把 case-specific 反馈不断堆进上下文。只有经过跨任务验证、能够解释一类问题且不会与其他场景冲突的原则，才由人以更抽象、更短的形式更新核心 Skill。
...
3. **report_builder Skill** 教宿主按 `report next / continue / submit / approve / feedback / revise` 协议执行当前 Manager 或 Worker 指令。

Worker 执行默认使用当前宿主的原生 sub-agent：WorkBuddy、Codex、Claude Code 分别选择 `workbuddy`、`codex`、`claude` adapter，由宿主自动判断，不向用户暴露技术选项。`inline` 只是不支持 sub-agent 时的显式兼容降级，不能静默使用。v0.4 不派发过程 Reviewer，由 Worker 自检和 runtime 确定性验证保证交付。
...
| Inline Run | 演示单步执行流程 |
| Agent Detail | 展示每个 Agent 的定位、输入输出契约、state 与 harness |
...
| `report next` | 获取当前 Manager/Worker 指令 | 返回统一的轻量 `current_instruction` |
| `report continue` | 自动消费已存在的输出 | 推进到人工 Gate、外部 Worker 或错误 |
...
| `report revise` | 显式指定返工阶段 | 保留已确认产物并局部重做 |
| `report status` | 查询项目、任务和当前 actor | 返回控制面摘要与 state/artifact 引用 |

`report start/next/submit/approve/feedback` 统一返回 `current_instruction`，不再因命令不同把
同一对象放入 `instruction` 或 `result`。默认响应不会内联完整 Manager state、Evidence
Catalog 或 Worker artifact；需要深度诊断时显式调用 `report manager-status`，业务执行则按
`instruction_path`、`input_path`、`output_path` 和 `artifact_path` 读取文件。
