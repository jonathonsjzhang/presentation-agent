...
3. **多Agent 和 Sub_agent机制**：当前的生成和 review 实际由同一个 agent 进行，在不同阶段使用不同skill，但存在上下文积累，可能导致上下文漂移问题。
4. **注入先验 Memory**：为系统预置一批经过验证的基础 memory（如战略汇报常见措辞偏好、高频 P0 红线等），让首次使用的用户也能获得一定的审查与风格引导，而非从零积累全部经验。
5. **项目模式迁移**：尝试如何将插件系统迁移到WB项目模式上。