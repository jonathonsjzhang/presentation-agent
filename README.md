...
1. **框架优化**
   - **可组合能力包**：将现有 Skill 拆成更小的原子能力，根据汇报对象、任务性质、材料格式和交付深度动态组合，减少无关 SOP、rubrics 和 schema 带来的上下文冗余。
   - **自动化评测**：搭建可重复运行的评测脚本，定义任务理解、论点质量、证据完整性、跨阶段一致性、格式交付和 Manager 调度效果等评测标准与实验场景。
