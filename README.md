...
这是面向 Codex、Claude Code、WorkBuddy 等 agent 终端的主接入方式，入口文件为 `skills/report_builder/SKILL.md`。其核心机制可以归结为一句话：**harness 不调模型，宿主调 harness。**
...
工具通过 GitHub 仓库分发，当前地址为 `https://github.com/jonathonsjzhang/presentation-agent`。

在 WorkBuddy、Codex 或 Claude Code 等宿主 Agent 终端中，发送以下指令即可完成安装：

> "帮我安装汇报助手，地址是 https://github.com/jonathonsjzhang/presentation-agent"

宿主 Agent 读取对应平台的 Host Adapter Skill 模板（位于 `templates/host_adapters/`），自动执行 `git clone`、`init-workspace` 和 `doctor` 自检，用户无需接触任何命令行。安装后各平台的使用方式完全一致，详细步骤见 `docs/GUIDEBOOK.md`。
...
### 7.4 Host Adapter Skill：一句"教我安装"就能接入

过去，宿主 Agent 需要开发者手动写一个厚重的 skill 文件，内含项目路径、复杂的内部协议和阶段定义。插件化改造后，每个平台的接入 skill 被压缩为一份薄模板，仅包含：

- 仓库地址获取逻辑（环境变量 / 用户提供 / 追问）
...
当前已为 WorkBuddy、Codex、Claude Code 三个平台各准备了一份模板（位于 `templates/host_adapters/`）。工具交付时，只需从对应模板复制一份到目标平台的 skill 目录，用户对 Agent 说一句"安装汇报助手"即可。
