...
1. 框架优化：
- **原生 Sub-agent Adapter**：当前已建立 Manager/Worker 任务隔离协议和目录隔离；下一步为各宿主增加原生 sub-agent 执行适配器，强化模型上下文物理隔离。
- **并行任务执行**：当前 Manager decision 一次派发一个任务；后续支持无依赖 task packet 的并发执行与汇合验收。
- **项目接入**：当前采用自行部署 Git 包 + skill的方式进行调用，方式复杂且不易于版本管理，需讨论如何接入WB官方插件库。

2. 知识迭代：
- **Skill 多轮迭代**：持续迭代 Manager 与 6 个 Worker 的 SOP 和 rubrics，并按汇报对象、性质和材料格式拆成可组合原子能力。
