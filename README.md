...
与此同时，`record_feedback()` 会向项目级事件流 `data/learning/events.jsonl` 同步记一笔（由 `LearningEventStore` 维护）。事件流是**跨 Agent 的横向流水账**，feedback / success / comparison / dreaming / retrieval / routing 都各记一条，用于全局观测，不参与生成。
