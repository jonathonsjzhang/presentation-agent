...
> 当前版本：`v0.4`（简化版主链；`v0.3` 作为兼容 profile 保留）
...
- **单一活动契约**：v0.4 是默认运行 profile，run state 会校验契约版本，避免错误恢复。
...
- `presentation_agent/loop.py`：`LoopRunner`，保留的 v0.3 调试入口；默认 v0.4 Manager 链不使用独立 Reviewer。
