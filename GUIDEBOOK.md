...
可同步补充保密要求、已有分析资料等。Agent 默认先生成完整文档；Analysis 完成后会给出 2-3 组主论点 + 分论点，并只用一个输入框确认：填写方案编号即可继续，也可以在同一处填写“都不好 + 原因”或直接写修改意见，不会再固定弹出第二个补充问题；需要修订时会回到同一个 Analysis agent 并再次确认。Storyline 完成后会展示一版故事线，首次只询问一个确认选择题；确认可以会直接进入 Report，只有选择重写或自行修改却未提供意见时才追问修改内容。文档完成后，再询问是否需要转为 PPT / HTML，或追加 QA list / 逐字稿。
