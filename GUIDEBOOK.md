...
以自然语言提出需求。Agent 会先进入 Brief 确认，主动追问研究目的、研究方向，并让你勾选高可信论据；其他信息会根据原始输入总结，缺少时使用预设值再请你确认。

| 信息 | 含义 | 默认 / 示例 |
|---|---|---|
| 研究目的 | 要回答的问题，或某话题的延伸汇报 | "AI 对广告行业的重塑会改变哪些竞争壁垒？" |
| 研究方向 | 论点 hypo，或希望引导的讨论/行动方向 | "判断明年是否加大 AI 广告产品投入" |
| 高可信论据 | 从 agent 整理的论据列表中勾选 | 用于判断子论点可信度和引用优先级 |
