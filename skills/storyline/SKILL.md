...
章节不是 findings 分类。每节只完成一个认知或证明动作；如果删掉某节，后文仍完全成立，该节通常不在主线。标题链单独阅读时，应能复述核心答案及其主要推导，形成论证累进；若只能用“另外、同时、还有”连接，仍是发现清单，不是故事线。

### 4. 标题表达判断，brief 说明故事

- `chapter`：简短主题名，只帮助定位。
- `heading`：本节最终建立的判断，也就是 leadline。写清业务对象和净判断，避免“背景、现状、原因、启示、建议”等目录式标题；只有本节承担 implication 任务时才写 implication，不要同时塞入前提、全部数据、边界和战略含义。
- `brief`：说明本节要完成的论证动作、最关键证据、必要解释或边界，以及它如何推进主线；不是正文预稿。
- `finding_refs`：只引用本节实际使用且真实存在的 finding ID。

`core_answer`、`heading` 和 `brief` 都要用面向最终读者的命题语言，不汇报 Worker 的证据审核过程。不要写“本批材料支持 A、现有证据不能支持 B”；若这一限制会改变判断，应改写为“A 在当前范围内可以观察到，B 的幅度、因果关系或适用阈值仍需通过相应数据或试点验证”。内部引用和 open issue 承担追溯与返工信息，主线只保留读者需要的判断边界。

承担相同证明作用的 findings 合并使用。低置信度 finding 只能承担假设或方向性解释；相关性不得写成因果，个案不得写成普遍规律。

标题链本身应能讲完主故事：读者只看 `core_answer` 和各节 `heading`，也能理解结论如何被逐步证明。证据边界应转译成面向读者的适用范围、条件或待验证事项，不写成 Worker 对自己材料和推理过程的审计说明。
...
正文措辞面向最终读者。不要写“本批材料支持的核心判断是”“现有证据能/不能支持”“基于上述 findings”等过程自述；如果证据有边界，直接把边界写进判断成立的范围、条件或后续验证要求。
