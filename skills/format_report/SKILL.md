---
name: format
description: Translate report.v1 into one target-specific formatted_material.v2 plan while preserving claims, evidence, caveats, and traceability; retain an explicit v0.2 page-content compatibility path.
---
...
本轮必须由 `delivery_target` 唯一选择 `format.document`、`format.ppt`、`format.html` 之一；不得从 legacy `output_format` 推断 v0.3 目标，也不得混用载体能力。
...
输入契约不匹配时停止并显式报错，绝不能把 `report.v1` 交给 legacy page-content 流程。
...
- 输入为 report.v1 却出现/依赖 legacy output_format；
- 为排版修改或删除上游结论；
- 丢失来源、口径、关键限定条件或阻断缺口；
- 只有格式建议，没有正式材料单元；
...
仅当输入 schema 为 `page_content.v1` / `page_content.v2`（或旧 `pages[]` / `draft_material.material_units[]`）时，执行原有 `page_content → formatted_material.v1` 流程，并使用 legacy `output_format` 选择唯一 capability。

Legacy 路径必须严格输出 `formatted_material.v1`，保留 `material_units`、`source_page_no`、`source_display`、`gap_display`、`style_tokens`、`format_decisions`、`open_design_tasks` 和 `downstream_handoff`。若收到 `report.v1`，必须拒绝，不得尝试把 sections 当 pages，也不得静默降级。
