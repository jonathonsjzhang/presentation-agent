...
- 每个 finding 是一张观点卡，不是一段小报告。`claim` 只表达一个清楚判断；关键数字可以保留，但不要把全部证据、解释、边界和战略含义塞进 claim。
- `so_what` 主要回答“这改变了对当前问题的什么理解”，不要把可能启发写成确定战略建议。只有材料和任务明确要求时，才写业务方向；即便写方向，也只写到判断含义，不替 Storyline 设计叙事或方案。
- 在思考中区分三类内容：直接回答当前任务的主发现；解释、支撑或限制主发现的支撑发现；回答相邻问题的发现。相邻问题即使有趣，也不要伪装成主 finding，应弱化为边界、`open_issues`，或在证据不足时舍弃。
- 同一组材料如果只能支撑一个局部观察，不要升级为跨问题结论；如果需要换一个问题才能成立，就说明它不属于当前任务主线。
...
明确分析主题、需要支持的决策和材料范围。区分用户希望证明的观点与材料已经支持的事实——不做预判。先判断当前任务要解释或判断什么，哪些只是相邻问题，避免后续把所有有趣发现都包装成主 finding。

### 2. 浏览材料并搭建分析框架，发散候选发现
