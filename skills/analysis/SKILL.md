...
- 一个 finding 至少能还原为“证据观察 → 有边界的推断 → 净判断 → so what”。`claim` 与 `so_what` 不能只是同一句话的改写。
...
- 只有材料能区分竞争性解释时，才写主要机制。优先检查样本选择、指标口径与时间窗、共同原因、反向因果、访谈偏差和外部事件；不要为了显得全面虚构反证。
- `confidence` 评价的是当前声明边界内的可信度：多源一致且关键替代解释已被削弱才可为 high；方向有支持但样本、口径或方法受限通常为 medium；少量访谈、单一来源或弱代理指标通常为 low。关键材料缺失使判断本身不能成立时应阻塞，而不只是降置信度。
...
优先形成能改变判断的比较，而不是堆单点事实；数字必须服务于明确主张。可以用“现象 → 拆解 → 机制 → 含义”推进，但只走到证据支持的最深一层，不把分析框架写入产物。
