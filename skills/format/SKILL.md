---
name: format
description: "[v1.0] Convert approved page content into a render-ready formal deliverable — professional consulting style, data authenticity, presentation specs. Schema-valid layout_type only."
---
# Format Skill v1.0
...
> ⚠️ **schema 约束**：`material_unit.layout_or_structure.layout_type` 必须为以下 schema 允许的值。
> 不在本表中的 layout 名会被 schema gate 拦截。

| `layout_type` (schema 允许) | 适用单元 | `hierarchy_map` 必含 | 触发 |
|---|---|---|---|
| `cover` | slide (cover) | `topic`, `subtitle` | 仅第 1 页 |
| `executive_summary` | slide | `primary` (1 句结论), `supporting` (≤3) | **必在 page 2（封面后一页）** |
| `key_takeaway` | slide (默认) | `takeaway` (1 句结论), `evidence` (1-2) | 大多数内容页 |
| `process_chevron` | slide | `stages[]` (3-5) | 结构性框架/金字塔 |
| `matrix_2x2` | slide | `quadrants[]` (4) | 战略定位 / 分类 |
| `horizontal_bar` | slide | `items[]` | 漏斗/转化/多柱对比 |
...
> **禁止类型**：`section_divider`（章节分隔页）和目录页不在本 layout 库中。
...
| # | `layout_type` (schema 映射) | 中文名 | vS 页码 | 触发场景 |
|---|---|---|---|---|
| 1 | `cover` | 封面页 | P1 | 报告首页，展示标题+日期+品牌 |
| 2 | `executive_summary` | 核心发现/执行摘要 | **P2（封面后一页）** | **必在封面后一页**，3-5个核心发现的概览，作为后续 slides 的核心观点总结 |
| 3 | `key_takeaway` | 方法论/背景说明 | P3 | 解释数据来源、方法论、定义关键概念 |
| 4 | `key_takeaway` | 可信性检验/数据校验 | P4 | 验证数据可信度（分布检验/一致性检查） |
| 5 | `dashboard_kpi_chart` | 数据概览/KPI仪表盘 | P5 | 展示核心指标总览（强留存率/样本量等） |
| 6 | `key_takeaway` | 图表+发现面板 ⭐最常用 | P6,P10,P15,P17-P19,P22 | **标准内容页**：一个主图表 + 右侧发现要点面板 |
| 7 | `data_table` | 表格+图表混合 | P7,P8 | 数据表格 + 配套柱状图的组合展示 |
| 8 | `grouped_bar` | 多柱对比图 | P9 | 三品牌在某维度上的横向对比 |
| 9 | `horizontal_bar` | 时间/分布图 | P9(下半) | 用户时间分布/占比分布的条形图 |
| 10 | `matrix_2x2` | 策略路线表 | P12 | 场景×功能×资源的矩阵式路线规划 |
| 11 | `key_takeaway` | 用户原声/引用 | P13 | 嵌入用户访谈原声 + 对应数据佐证 |
| 12 | `horizontal_bar` | 转化/迁移矩阵 | P14 | 用户迁移路径/转化效率的多维对比 |
| 13 | `closing` | 讨论/行动建议 | P23 | 报告末页，Exploit/Explore 分类的行动建议 |
