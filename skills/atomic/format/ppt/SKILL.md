...
- **版式按内容类型匹配，禁止所有页面使用同一版式**：

  | 内容类型 | 推荐版式 | 占比参考 |
  |---------|---------|---------|
  | 数据分析（单指标） | 方案A: 左图/表 + 右发现面板/散点联动 | ~40% |
  | 多维对比（多产品/多维度） | 方案B: 三栏/分组图 + 侧栏注释 | ~25% |
  | 方法论/数据说明 | 方案C: 上文字 + 下图/表 | ~15% |
  | 战略/行动建议 | 方案D: 结构化卡片/矩阵 | ~15% |
  | 纯文字(仅限少数) | 结构化形状卡片，禁止纯段落 | ~5% |
