...
**唯一对外交付物是一份 Markdown 文件**——完整的、可独立阅读的战略分析报告草稿，含 executive_summary 和全部正文。下游 Format worker 基于这份 Markdown 生成 DOCX / PPT / HTML。JSON 仅作为内部追溯数据并行输出，不面向读者。
...
- `evidence_refs` 只能来自所引 finding 已声明的 refs。所有关键数字和引用在 Markdown 中以 `(来源：xxx)` 标注，完整的来源登记写入 JSON 的 `source_registry`。
...
### 4. 写尾部
- **可追溯引用**：所有引用来源登记进 JSON 的 `source_registry`，Markdown 中来源以 `(来源：xxx)` 标注在正文语句末尾。
- **so what 收束**（准则四）：确认每节收尾都有落脚点，追问了下一层问题；涉及抓手比较时用"空间侧 + 收益侧"讲清比较后的 so what；全篇结论收束为一句可复述的判断 + 分领域关注点。
- 填 JSON 的 `caveats_and_limits`、`recommendations`、`format_handoff`。Markdown 的「方法与边界」「建议」「附录」三个章节对应这些内容。
...
输出两份文件：
- **Markdown 报告** — 交付物，供人类阅读和下游 Format 消费。结构见下方 Output 段。
- **`report.v1` JSON** — 内部追溯数据，含 source_registry、caveats_and_limits、format_handoff 等元数据。不面向读者。

---

## Output

### Markdown 报告（交付物，下游 Format 的唯一输入）

一份可直接阅读的 Markdown 文件，含完整叙事和注释。Format worker 基于它生成 DOCX / PPT / HTML。
...
## 一、章节标题
（连续段落正文。论据来源标注为 (来源：xxx)，方法与口径注释用 > 引用块或小字，
  边界说明融在正文括号内，图表处标注 [建议图表：xxx]）

## 二、章节标题
...

## 方法与边界
（caveats_and_limits 的完整内容，含方法说明、口径定义、数据缺口、关键假设、反方观点）

## 建议
（方向性建议，每条含 rationale + 成立条件）

## 附录
（补充证据、非主线 finding，内容可读而非仅有 ID）
```

### 编排纪律（写 Markdown 前必读）

以下规则确保 Markdown 是连续可读的叙事，而不是 ID 和警告块的碎片拼贴：

**1. 边界说明融进正文，不单开警告块。** 短小的边界说明（样本小、自报偏差、时间窗口差异）写成括号从句，同段收尾。只有整节级别的重大边界（如"本分析仅基于 App 端数据，PC/Web 端未知"）才值得独立成段。每节最多 1–2 处独立边界段。
...
> 证据 E002、E014
```
——每段正文后跟一串 ID 块，叙事被反复打断。

正确做法：边界写成括号内从句融在正文里，来源标在语句末尾 `(来源：xxx)`。claim ID 和 evidence ID **永远不出现在 Markdown 正文中**——它们是内部追溯数据，只存在于 JSON 的元数据字段里。

**2. Transition 和 conclusion 是普通段落。** 每节末尾的过渡句和结论就是散文，按正常段落写，不加标签、不挂特殊样式。

**3. 附录内容可读。** 不写 `关联内容：E035` 这种对读者无意义的 ID 引用。附录的每个条目要写出实际内容（来源描述、补充发现的具体文字）。

**4. 图表建议对读者有意义。** `[建议图表：DS/豆包/元宝三家人均使用时长趋势对比折线图]` 是好的——读者一看就知道这里会有什么图。`[建议图表：根据 E003 数据绘制]` 是坏的——读者不知道这是什么。

### JSON（内部追溯数据，并行输出）

Markdown 交付物之外，并行输出一份 `report.v1` JSON，承载以下仅供程序消费的元数据：

- `agent_id` / `schema` — 固定值
- `report_metadata` — `title`、`topic`、`audience`、`report_type`
- `source_registry[]` — 所有引用来源的完整登记，供后续核查
- `caveats_and_limits` — 方法、口径、假设、数据缺口、边界合并一处
- `recommendations[]` — 方向性建议的结构化版本
- `format_handoff` — `protected_caveats`（需保护的边界声明）+ `visual_opportunities`（值得可视化的机会清单）

JSON 中的 `claim_ids`、`evidence_refs`、`source_id` 等 ID 字段**不出现在 Markdown 中**——它们只存在于 JSON 里，供程序追溯。
