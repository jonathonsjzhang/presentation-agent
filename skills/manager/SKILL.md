...
5. `input_artifacts` 使用 Manager Context 中真实存在的路径，不得虚构。固定下一跳和正式 artifact 路径由 runtime 最终绑定；不要引用 Worker 临时的 `handoff/output_*.json`。
...
- Storyline 每章是否已明确关键证据、推导关系和判断边界，使 Report 无需重新做分析取舍？
...
Brief 页数是目标，不是零容差截断线。runtime 会自动允许最多多 1 页，并以真实渲染页数而非字符估算作为最终依据。仍超过自动容差时，runtime 打开 `page_budget` human gate：用户选择接受当前篇幅后更新项目上限并继续；用户选择继续收窄时复用当前 Report/Format task 返工。Manager 不得替用户静默扩大上限，也不得在这个 gate 前无限重复压缩。
...
Storyline 通过后，runtime 会在进入 Report 前暂停，只展示一版 `storyline.md`：核心答案、Executive Summary、章节故事线、关键边界和不进入主线的内容。
...
- Acceptance：只需输出 `action` + `acceptance_report`。dispatch/revise 的 `task_packet` 可以省略；runtime 会根据固定链、当前正式 artifact 和返工要求生成或规范化 task packet。
...
需要输出 `task_packet` 时，只保留 agent_id、objective、input_artifacts，以及返工时的 revision_feedback；其中下一阶段 agent_id 和 input_artifacts 仍以 runtime 规范化结果为准。

`acceptance_report` 只保留 verdict、reason，以及返工时的 revision_requirements。确定性结构检查由 runtime 完成，不要求 Manager 重写检查明细。
